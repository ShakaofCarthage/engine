...
/**
 * Implements the Fleet Patrol Movement order.
 */
public class FleetPatrolMovement
        extends FleetMovement {

    /**
     * a log4j logger to print messages.
     */
    private static final Logger LOGGER = LogManager.getLogger(FleetPatrolMovement.class);

    /**
     * Type of the order.
     */
    public static final int ORDER_TYPE = ORDER_P_FLEET;
     ...
     */
    public FleetPatrolMovement(final OrderProcessor myParent) {
        super(myParent);
        LOGGER.debug("FleetPatrolMovement instantiated.");
    }
     ...
     */
    protected void performMovement(final Fleet theUnit, final List<Sector> lstPosSectors) {
        int availMps = getMps(theUnit);

        final List<Ship> patrolShips = ShipManager.getInstance().listByFleet(getOrder().getGame(), theUnit.getFleetId());
